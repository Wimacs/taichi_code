...
dye = ti.field(dtype=ti.f32, shape=(nx, ny))
dye_temp = ti.field(dtype=ti.f32, shape=(nx + 1, ny))

dye2 = ti.field(dtype=ti.f32, shape=(nx, ny))
dye_temp2 = ti.field(dtype=ti.f32, shape=(nx + 1, ny))
...
def add_force(dt: ti.f32):
    ...
    # for i, j in ux:
    #     ux[i, j] -= gravity * dt
    #     if i == 2 and 110 < j < 140:
    #         dye[i, j] = 1
    #     #dye[i, j] *= 0.995

    force_radius = N / 3.0
    inv_force_radius = 1.0 / force_radius
    inv_dye_denom = 4.0 / (N / 15.0)**2
    f_strength_dt = 2000 * dt
    for i, j in dye:
        omx, omy = 0.01*N, 0.5*N
        mdir = 1
        dx, dy = (i + 0.5 - omx), (j + 0.5 - omy)
        d2 = dx * dx + dy * dy
        # dv = F * dt
        factor = ti.exp(-d2 * inv_force_radius)
        momentum = mdir * f_strength_dt * factor * 3
        v = ux[i, j]
        ux[i, j] = (v + momentum)
        dc = dye[i, j]
        dc += ti.exp(-d2 * inv_dye_denom) * 1

        dye[i, j] = dc * 0.977
    
    f_strength_dt = 2000 * dt
    for i, j in dye2:
        omx, omy =0.8*N,  0.02*N
        mdir = 1
        dx, dy = (i + 0.5 - omx), (j + 0.5 - omy)
        d2 = dx * dx + dy * dy
        # dv = F * dt
...
def substep(dt):
    ...
    advect(dye_temp, dye, dt, 0, 0, nx, ny)
    advect(dye_temp2, dye2, dt, 0, 0, nx, ny)
    ux.copy_from(ux_temp)
    uy.copy_from(uy_temp)
    dye.copy_from(dye_temp)
    dye2.copy_from(dye_temp2)
    apply_bc()
...
def init_dambreak():
    for i, j in pressure:
        ...
        divergence[i, j] = 0
        marker[i, j] = FLUID
        if ti.Vector([i - circle_pos[0], j - circle_pos[1]]).norm() < circle_r:
            marker[i, j] = SOLID
        if (i == 0 or i == nx-1 or j == 0 or j == ny-1) \
                or (j == ny * 2 // 3 and i < nx * 2 // 3) \
...
def fill_color():
    for i, j in color_buffer:
        color_buffer[i, j] = ti.Vector([dye[i, j], 0, dye2[i, j]])
        if marker[i, j] == SOLID:
            color_buffer[i, j] = ti.Vector([0, 1, 0])
...
def main():
    ...
    cnt = 0
    gui = ti.GUI("demo", res=(res_x, res_y))
    while viewer.active():
